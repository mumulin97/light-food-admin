  ...
  <div class="store-management-content" v-loading="useBackend && loading">
    ...
    <section class="store-table-card">
      <el-table :data="pageStores" class="store-management-table" table-layout="fixed" empty-text="暂无符合条件的门店">
        ...
        <el-table-column label="操作" width="82" align="left" class-name="table-op-column" label-class-name="table-op-column"><template #default="{ row }"><div class="table-row-actions"><el-button class="table-icon-button" circle aria-label="编辑门店" @click="openEdit(row)"><AppIcon name="edit"/></el-button><el-button class="table-icon-button" circle aria-label="查看库存" @click="openInventory(row)"><AppIcon name="settings"/></el-button></div></template></el-table-column>
      </el-table>
