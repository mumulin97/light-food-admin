  ...
  <div class="store-management-content" v-loading="useBackend && loading">
    ...
    <section class="store-table-card">
      <el-table :data="pageStores" class="store-management-table" table-layout="fixed" empty-text="暂无符合条件的门店">
        ...
        <el-table-column label="负责人" min-width="96"><template #default="{ row }"><el-tooltip placement="top" :show-after="220" popper-class="store-detail-tooltip"><template #content><div class="store-tooltip-content"><span><AppIcon name="users"/></span><div><small>门店负责人</small><strong>{{ row.manager }}</strong></div></div></template><span class="store-detail-text" tabindex="0" :aria-label="`门店负责人：${row.manager}`">{{ row.manager }}</span></el-tooltip></template></el-table-column>
        <el-table-column prop="phone" label="电话" width="118" show-overflow-tooltip />
        <el-table-column label="状态" width="86" align="left"><template #default="{ row }"><span class="store-status" :class="row.status === '营业中' ? 'open' : 'closed'">{{ row.status }}</span></template></el-table-column>
        <el-table-column label="操作" width="168" align="left" class-name="table-op-column store-op-column" label-class-name="table-op-column store-op-column"><template #default="{ row }"><div class="table-row-actions"><button type="button" class="table-action-link" @click="openEdit(row)">编辑</button><button type="button" class="table-action-link" @click="openInventory(row)">查看库存</button></div></template></el-table-column>
      </el-table>
